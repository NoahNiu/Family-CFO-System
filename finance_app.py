...
# --- 1. 页面配置与安全门禁 ---
st.set_page_config(page_title="Balance & Future Pro V15.2", layout="wide")
...
with st.sidebar.expander("💰 方案 A：当前现实", expanded=True):
    st.write(f"**{n_a}**")
    a_g = st.number_input(f"{n_a}主业税前薪资", value=26000.0)
    a_gig = st.number_input(f"{n_a}副业月收入", value=0.0)
    a_h = st.number_input(f"{n_a}月均工时基准", value=176)
    st.write(f"**{n_b}**")
    b_g = st.number_input(f"{n_b}主业税前薪资", value=18000.0)
    b_gig = st.number_input(f"{n_b}副业月收入", value=0.0)
    b_h = st.number_input(f"{n_b}月均工时基准", value=176)
    pf_pct = st.slider("公积金比例(%)", 5, 12, 5) / 100
    st.write("---")
    m_p = st.number_input("房贷本金总额", value=400000.0)
    m_r = st.number_input("房贷利率(%)", value=3.2)
    m_y = st.number_input("还款年限(年)", value=30)
    asset_now = st.number_input("当前家庭总资产", value=600000.0)
    c_l = st.number_input("每月车贷支出", value=2000.0)
    living = st.number_input("家庭基础生活费", value=6000.0)
    other = st.number_input("其他教育/杂项支出", value=0.0)
...
if show_b:
    with st.sidebar.expander("✨ 方案 B：变量预测", expanded=True):
        ...
        living_b = st.number_input("预期生活费变动(B)", value=living)
        purchase_b = st.number_input("预期年度大额开销(B)", value=0.0, help="折算到每月的金额")
else:
...
if 'act_member' not in st.session_state: st.session_state.act_member = n_a
# 修复：在此处计算全局通用的 active_hourly
active_hourly = (a_g + a_gig)/a_h if st.session_state.act_member == n_a else (b_g + b_gig)/b_h
...
if view == "🏠 Balance 看板":
    ...
    moat_months = asset_now / data_a['total_exp'] if data_a['total_exp'] > 0 else 0
    ...
    col1.metric("家庭总到手(A)", f"¥{int(data_a['total_net']):,}")
    col2.metric("家庭总支出(A)", f"¥{int(data_a['total_exp']):,}")
    col3.metric("本月净结余(A)", f"¥{int(data_a['savings']):,}", delta=f"{int(save_rate*100)}% 储蓄率")
    col4.metric(f"{st.session_state.act_member} 实时时薪", f"¥{active_hourly:.1f}")
    
    st.info(f"💡 **公积金抵消感增强：** 每月入账 ¥{int((data_a['res_a']['fund'] + data_a['res_b']['fund']) * 2):,}，已自动对冲了房贷月供的 **{int(((data_a['res_a']['fund'] + data_a['res_b']['fund']) * 2)/data_a['curr_m']*100) if data_a['curr_m']>0 else 100}%**。")
    
    if data_a['savings'] > 0:
        travel_total = dream_cost * travel_num
        st.success(f"🏖️ **家庭旅行自由指数：** 按目前结余，每 **{travel_total/data_a['savings']:.1f}** 个月可全款去一次{dream_dest}。")

    st.divider()
    st.subheader("🔍 消费工时透视")
    l1, l2 = st.columns([1, 2])
    with l1:
        st.selectbox("谁来买单？", [n_a, n_b], key="act_member")
        item_name = st.text_input("心仪商品/梦想支出", "新智能手机")
        price = st.number_input("商品价格(元)", value=6000.0)
    with l2:
        h = float(price / active_hourly) if active_hourly > 0 else 0
        st.write(f"购买 **{item_name}** 相当于消耗 **{st.session_state.act_member}** 约 **{h:.1f} 小时** 的奋斗成果。")
    ...
    target_a = data_a['total_exp'] * 12 * 25 if fire_mode == "25倍年支原则 (科学默认)" else st.number_input("自定义退休金目标", value=5000000.0)
    target_b = data_b['total_exp'] * 12 * 25 if fire_mode == "25倍年支原则 (科学默认)" else target_a
    
    f1, f2 = st.columns(2)
    with f1:
        st.metric("方案 A FIRE 目标", f"¥{int(target_a):,}", delta=f"达成率 {asset_total/target_a*100:.1f}%")
        st.progress(min(asset_total/target_a, 1.0))
    with f2:
        if show_b:
            st.metric("方案 B FIRE 目标", f"¥{int(target_b):,}", delta=f"达成率 {asset_total/target_b*100:.1f}%")
    ...
    if show_b:
        ...
        df_p = pd.DataFrame(proj_a + proj_b)
        st.plotly_chart(px.line(df_p, x="月", y="月支出", color="方案"), use_container_width=True)
    else:
    ...
    **💡 这个页面想告诉你：**
    ...
    if 'subs' not in st.session_state:
        st.session_state.subs = pd.DataFrame([
            {"项目": "视频会员", "月费": 35.0, "状态": True},
            {"项目": "云空间", "月费": 21.0, "状态": True},
            {"项目": "健身外卖红包", "月费": 100.0, "状态": True}
        ])
    ...
    # 修复：不再依赖局部变量，直接使用全局计算的 active_hourly
    total_bh = (daily * 365 + monthly_sub * 12) * y
    
    st.error(f"😱 **断头台报告：** {y} 年后你将累计消耗 ¥{total_bh:,.0f}，相当于你奋斗了 **{total_bh/active_hourly:.1f} 小时** 的奋斗成果。")
